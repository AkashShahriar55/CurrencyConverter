package com.akash.currencyconverter.data.database.worker
...
import com.akash.currencyconverter.data.database.databases.AppDatabase
            ...
            if (filename != null) {
                applicationContext.assets.open(filename).use { inputStream ->
                    JsonReader(inputStream.reader()).use { jsonReader ->
                        ...
                        val unitType = object : TypeToken<List<BalanceEntity>>() {}.type
                        val unitList: List<BalanceEntity> = Gson().fromJson(jsonReader, unitType)

                        val database = AppDatabase.getInstance(applicationContext)
                        database.balanceDao.insert(unitList)
